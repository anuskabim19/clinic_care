...
import 'package:clinic_app/features/data/models/doctor_model.dart';
import 'package:clinic_app/features/data/models/hospital_model.dart';
import 'package:clinic_app/features/data/models/my_chat_model.dart';
import 'package:clinic_app/features/data/models/text_message_model.dart';
...
class FirebaseRemoteDataSourceImpl implements FirebaseRemoteDataSource {
  ...
  FirebaseRemoteDataSourceImpl({
    ...
    required this.auth,
    required this.googleSignInAuth,
  });

  @override
  Future<void> signIn(String email, String password) async => await auth.signInWithEmailAndPassword(
  ...
  @override
  Future<void> getCreateCurrentUser(dynamic user) async {
    final userCollection = fireStore.collection("users");
  ...
  final userCollection = fireStore.collection("users");
  return userCollection.snapshots().map<List<UserEntity>>((querySnapshot) =>
      querySnapshot.docs.map<UserEntity>((e) => UserModel.fromSnapshot(e)).toList());
}
  ...
  final hospitalRef = fireStore.collection("hospital");

  return hospitalRef.snapshots().map((querySnap) =>
      querySnap.docs.map((queryDoc) => HospitalModel.fromSnapshot(queryDoc)).toList());
}
  ...
  Future<void> sendMessage(TextMessageEntity textMessageEntity, String channelId) async{
    ...
    final newMessage = TextMessageModel(
      ...
      senderName: textMessageEntity.senderName,
      time: textMessageEntity.time,
      isOdp: textMessageEntity.isOdp, senderUID: '',
    ).toDocument();
